...
function generat_head_cell(text,class_s,data_column,id,tashkeel_flage){
	...
	return "<th id=\""+id+"\" class=\""+class_s+" table_char  no_pading stickyheader hov_cell character_text\" data-column=\""+data_column+"\">"+text+"</th>"
    }
...
function show_all_info_mode(){
	...
	all_body += "<td class=\"column100 column_"+(j+3)+" char_cell\" data-column=\"column"+(j+3)+"\">"+alphabet_content+"</td>"
    }
    //merge all html
...
function generat_sura_row(sura_info,start_pos,end_pos){
  ...
  for(j = start_pos; j < end_pos; j++){
    ...
    table_content += "<tr id=\""+word_id+"\" class=\"word"+j+" hov_cell word\"><td>"+ (j+1) + " : " + sura_info.unique_words[j]+"</td></tr>"
  }
  content.push(table_content)
  ayat_small_table = "<table class=\""+sura_info.sura_name.replace(' ','')+"_table word\">"+table_content+"</table>"
  sura_body = "<td class=\"column100 column_1 "+sura_info.sura_name.replace(' ','')+" char_cell\" data-column=\"column1\">"+ayat_small_table+"</td>"
  //*****************************************************************
  ...
  for(j = 0; j < sura_info.ayat_num; j++){
    ...
    for(n = start_pos; n < end_pos; n++){
      ...
      table_content += "<tr id=\""+table_char_id+"\" class=\"word"+n+" column100 char_counts_table table_char hov_cell "+sura_id+"_word_"+n+" clicked"+j+"\"><td class=\"column100\">"+aya.frequency[n]+"</td></tr>"
    }
    content.push(table_content)
    alphabet_content = "<table class=\""+sura_info.sura_name.replace(' ','')+"_table\">"+table_content+"</table>"
    sura_body += "<td class=\"column100 column_"+(j+2)+" "+sura_info.sura_name.replace(' ','')+" char_cell\" data-column=\"column"+(j+2)+"\">"+alphabet_content+"</td>"
  }
  //*****************************************************************

  //merge all html
  sura_html = "<tr class=\"row100 "+sura_info.sura_name.replace(' ','')+"\">" +sura_body+ "</tr>"
...
$(document).ready(function() {
  ...
  win.scroll(function() {
    ...
    if(Math.abs($('.table100').height() - win.height()) < win.scrollTop() && !all_info_mode){
      ...
      if (!the_end_of_ayat)
      {
        ...
        if((end_render+step_render <= Quran_.swar[curent_sura_num].unique_words.length)){
          ayat_content_ = generat_sura_row(Quran_.swar[curent_sura_num], end_render, end_render+step_render)[1]
          for( i=0; i<ayat_content_.length; i++){
            $(".column_"+(i+1)).find("."+Quran_.swar[curent_sura_num].sura_name.replace(' ','')+"_table").children('tbody').append(ayat_content_[i])
          }
        ...
        //check if no enough ayat to genrate
          ...
          if( Quran_.swar[curent_sura_num].unique_words.length != end_render ){
            ayat_content_ = generat_sura_row(Quran_.swar[curent_sura_num], end_render, Quran_.swar[curent_sura_num].unique_words.length)[1]
            for( i=0; i< ayat_content_.length; i++){
               $(".column_"+(i+1)).find("."+Quran_.swar[curent_sura_num].sura_name.replace(' ','')+"_table").children('tbody').append(ayat_content_[i])
            }
        ...
        if(check_if_append_nodes_down && ayat_in_table >= 16+step_render){
          children_nodes = $(".column_"+(1)).find("."+Quran_.swar[sura_num_to_remove_from_top].sura_name.replace(' ','')+"_table").children('tbody').children('tr')
          if (children_nodes.length > step_render){
            for (i=0; i< (Quran_.swar[curent_sura_num].ayat_num+1); i++){
              children_nodes = $(".column_"+(i+1)).find("."+Quran_.swar[sura_num_to_remove_from_top].sura_name.replace(' ','')+"_table").children('tbody').children('tr')
              children_nodes.slice(0,step_render).remove();
            }
          }
          else{
            num_of_rest_nodes = step_render - children_nodes.length
            $(".swar").find("."+Quran_.swar[sura_num_to_remove_from_top].sura_name.replace(' ','')).remove();
      ...
      if (!the_begin_of_ayat){
        ...
        if( first_aya > step_render){
          ...
          for( i=0; i< ayat_content.length; i++){
            $(".column_"+(i+1)).find("."+Quran_.swar[sura_num_to_remove_from_top].sura_name.replace(' ','')+"_table").children('tbody').prepend(ayat_content[i])
          }
          ayat_in_table += step_render
          num_of_steps_up += 1
        }
        ...
        if(num_of_steps_up == 1 || ayat_in_table >= 16+step_render){
          children_nodes = $(".column_"+(1)).find("."+Quran_.swar[curent_sura_num].sura_name.replace(' ','')+"_table").children('tbody').children('tr')
          children_nodes_len = children_nodes.length
          if (children_nodes_len > step_render){
            for (i=0; i< (Quran_.swar[current_sura-1]+1); i++){
              children_nodes = $(".column_"+(i+1)).find("."+Quran_.swar[curent_sura_num].sura_name.replace(' ','')+"_table").children('tbody').children('tr')
